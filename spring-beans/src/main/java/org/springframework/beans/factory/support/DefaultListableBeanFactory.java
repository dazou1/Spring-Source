...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	public <T> T getBean(Class<T> requiredType, @Nullable Object... args) throws BeansException {
		// NamedBeanHolder 可以理解为和 map 数据结构差不多的东西，里面存放的就是 bean 的名字和 bean 实例
		// 通过这个方法 resolveNamedBean，就会对 bean 对象进行了代理，需要详细查看该方法是如何实现动态代理的
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		// 所有bean的名字，同时也存储在了 beanDefinitionMap 中，从这个 Map 中根据 bean 的名字拿出描述bean的BeanDefinition进行实例化bean
		// beanDefinitionNames 存储了所有需要被 Spring 管理的类的 beanName
	...
	public void registerBeanDefinition(String beanName, BeanDefinition beanDefinition)
		...
		if (oldBeanDefinition != null) {
			...
			if (hasBeanCreationStarted()) {
				 ...
				 * 里面放着beanDefinitionMap，beanDefinitionNames，beanDefinitionMap是一个hashMap，
				 * beanName作为Key,beanDefinition作为Value，
